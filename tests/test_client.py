...
class ClientTest(unittest.TestCase):
    ...
    def setUp(self):
        self.test_pandas_data = pd.read_csv(
            os.path.join(FILE_PATH, "examples/test.csv")
        )
        self.test_astropy_data = Table.read(
            os.path.join(FILE_PATH, "examples/test.votable"), format="votable"
        )

    @mock.patch(
        "cds_xmatch_client.requests.post",
        return_value=MockResponse(
            content_path=os.path.join(FILE_PATH, "examples/res.txt"), format="pandas"
        ),
    )
    ...
    @mock.patch(
        "cds_xmatch_client.requests.post",
        return_value=MockResponse(
            content_path=os.path.join(FILE_PATH, "examples/res.txt"), format="votable"
        ),
    )
    ...
    @mock.patch("cds_xmatch_client.requests.post")
